...
namespace DB2FileReaderLib.NET
{
    public class WDB3Row : IDB2Row
    {
        ...
        public WDB3Row(DB2Reader reader, BitReader data, int id, int recordIndex)
        {
            m_reader = reader;
            m_data = data;
    ...
    public class WDB3Reader : DB2Reader
    {
        ...
        {
            { 3348406326, DB2Flags.Sparse }, // conversationline
            { 2442913102, DB2Flags.Sparse }, // item-sparse
            { 2982519032, DB2Flags.Sparse | DB2Flags.SecondaryKey }, // wmominimaptexture
        };
        ...
        public WDB3Reader(Stream stream)
        {
            using (var reader = new BinaryReader(stream, Encoding.UTF8))
            {
                ...
                for (int i = 0; i < RecordsCount; ++i)
                {
                    ...
                    IDB2Row rec = new WDB3Row(this, bitReader, Flags.HasFlagExt(DB2Flags.Index) ? m_indexData[i] : -1, i);
